<div class="main-sidebar sidebar-style-2">
        <aside id="sidebar-wrapper">
          <div class="sidebar-brand">
            <img src="images/logo2.png" width="60%" >
          </div>
          <div class="sidebar-brand sidebar-brand-sm">
          <img src="images/logo.png" width="60%">
          </div>
          <ul class="sidebar-menu" id="sidebar-menu">

            <li class="active"><a class="nav-link" href="#dashboard"><i class="fas fa-fire"></i> <span>Dashboard</span></a></li>
            
            <li class="">
              <a href="#kendaraan" class="nav-link"><i class="fas fa-car"></i><span>Kendaraan</span></a>
            </li>
            
            <li class="">
              <a href="#pelanggan" class="nav-link"><i class="fas fa-user"></i> <span>Pelanggan</span></a>
            </li>
            <li class="">
              <a href="#pembelian" class="nav-link" ><i class="fas fa-file"></i> <span>Pembelian</span></a>
            </li>
            <li class="">
              <a href="#laporan" class="nav-link"><i class="fas fa-folder"></i> <span>Laporan</span></a>
            </li>
            <li class="">
              <a href="#administrasi" class="nav-link" ><i class="fas fa-users"></i> <span>Administrasi</span></a>
            </li>
        
       
        </aside>
      </div>